...
val_list = []
random.seed(42)
path_to_dataset = "C:\\Users\\Marko\\Desktop\\TekMology\\Zavrsni\\CGHD-1152-valid"
...
j = 0
while j < 50:
    val_list.append(random.choice(img_only))
    j += 1
...
model = torch.hub.load("yolov5-master", 'custom',
                       path="best_weights.pt", source='local')  # local repo
for i, filename in enumerate(dirlist):
    img = cv.imread(f"C:\\Users\\Marko\\Desktop\\TekMology\\Zavrsni\\CGHD-1152-valid\\{filename}")[..., ::-1]  # image loader
    results = model(img, size= 1280)  # inference
    ...
    # New file
    final_file = open(f"validation_res\\{filename}.asc", "w")
    final_file.writelines(no_duplicates)
 

    fo.close()
    os.remove("ltspice_data.asc") #!    
    final_file.close()
